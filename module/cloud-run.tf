...
#    service_account = google_service_account.cloudrun_service_account.email
    containers {
      name = "${var.prefix}-bankapp"
      image = "${google_artifact_registry_repository.google_container_registry.registry_uri}/${var.IMAGE_NAME}:${var.TAG_NUMBER}"   ###"${var.REPO_NAME}/${var.IMAGE_NAME}:${var.TAG_NUMBER}"
      ports {
        container_port = 8080
      }
      resources {
